...
class VehicleModel(BaseModel):
    ...
    vehicle_id: Any = Field(None, description="车辆ID（根据车牌号hash后生成）")
    ...
    vehicle_driver_name: Any = Field(None, description="司机姓名（必须有）")
    vehicle_type: Any = Field(None, description="车辆类型（目前有餐厅收集车to_rest、销售运输车to_sale两种）")
    
    vehicle_rough_weight: Any = Field(None, description="毛重（根据不同的类型有计算公式）")
    vehicle_tare_weight: Any = Field(None, description="皮重（根据不同的类型有计算公式）")
    vehicle_net_weight: Any = Field(None, description="净重（毛重-皮重）")
